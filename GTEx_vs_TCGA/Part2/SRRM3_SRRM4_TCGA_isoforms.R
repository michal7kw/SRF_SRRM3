# Comprehensive analysis of SRRM3 isoforms and SRRM4 across TCGA tumor types
library(recount3)
...
calculate_psi <- function(junction_data) {
  ...
  # Identify inclusion junctions (must splice to/from exon 15)
  inclusion_jxn <- which(
    (junctions$end_pos == SRRM3_INFO$exon15$start) |  # upstream junction
    (junctions$start_pos == SRRM3_INFO$exon15$end)    # downstream junction
  )
  
  # Identify exclusion junctions (skip over exon 15)
  exclusion_jxn <- which(
    junctions$start_pos < SRRM3_INFO$exon15$start &
    junctions$end_pos > SRRM3_INFO$exon15$end
  )
