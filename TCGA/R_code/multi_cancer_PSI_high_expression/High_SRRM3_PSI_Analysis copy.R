...
# Set the cancer type environment variable
cancer_type <- "ACC"
...
rse_gene <- create_rse(project_info[1,], type = "gene", annotation = "gencode_v26")
...
message("Available colData columns: ", paste(head(names(colData(rse_gene))), collapse=", "))
...
any(duplicated(expression_data$case_id))
expression_data$expression <- as.numeric(as.character(expression_data$expression))
cutoff <- quantile(expression_data$expression, probs = threshold, na.rm = TRUE)
...
setwd("/beegfs/scratch/ric.broccoli/kubacki.michal/SRF_SRRM3/TCGA/R_code/multi_cancer_PSI_high_expression")

# Get clinical data from GDC, handling potential data size mismatch
# clinical_raw <- GDCquery_clinic(project = paste0("TCGA-", cancer_type), type = "clinical", save.csv = TRUE)
...
follow_up$case_submitter_id
# Debug: Print unique values before conversion
message("Unique days_to_death values:")
print(unique(clinical$days_to_death))
message("Unique days_to_follow_up values:")
print(unique(latest_follow_up$days_to_follow_up))
