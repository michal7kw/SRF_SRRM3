...
# Modify get_extended_clinical_data function to use TCGAbiolinks approach
get_extended_clinical_data <- function(cancer_type) {
  ...
  tryCatch({
    ...
    flog.info("Retrieved clinical data with %d patients", nrow(clinical))
    
    # Process survival information
    clinical <- clinical %>%
      mutate(
        status = ifelse(vital_status == "Alive", 0, 1),
        survival_time = ifelse(vital_status == "Alive",
                             days_to_last_follow_up,
                             days_to_death)
      )
    
    # Print data summaries
    flog.info("Clinical data summary:")
    print(summary(clinical[c("survival_time", "status")]))
    
    flog.info("Vital status distribution:")
    print(table(clinical$vital_status, useNA = "ifany"))
    ...
    if (nrow(clinical) == 0) {
      flog.error("No valid samples after filtering missing data")
      return(NULL)
    }
    
    flog.info("Final number of samples with valid survival data: %d", nrow(clinical))
...
prepare_analysis_data <- function(values, groups, clinical_data) {
  ...
  # Match clinical data using sample IDs
  # Convert TCGA barcodes to match between datasets if needed
  analysis_data$sample_id <- substr(analysis_data$sample_id, 1, 12)
  clinical_data$sample_id <- substr(clinical_data$submitter_id, 1, 12)
  
  # Merge with clinical data
  analysis_data$survival_time <- clinical_data$survival_time[match(analysis_data$sample_id, clinical_data$sample_id)]
  analysis_data$status <- clinical_data$status[match(analysis_data$sample_id, clinical_data$sample_id)]
  ...
  flog.info("Final analysis data has %d samples", nrow(analysis_data))
