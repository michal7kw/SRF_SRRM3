...
###############################################################################
# High SRRM3 PSI Analysis by Grade SLURM Job Script
#
# This script performs PSI analysis on samples with high SRRM3 expression across
# multiple cancer types, stratified by tumor grade, using SLURM array jobs.
###############################################################################

#SBATCH --job-name=High_SRRM3_PSI_Grade
...
#SBATCH --error="logs/High_SRRM3_PSI_Analysis_Grade_%a.err"
#SBATCH --output="logs/High_SRRM3_PSI_Analysis_Grade_%a.out"
#SBATCH --array=0-5
...
mkdir -p logs
mkdir -p results/high_srrm3_analysis_by_grade
mkdir -p cache
...
echo "Starting high SRRM3 PSI analysis by grade for ${CANCER_TYPE} at $(date)"

# Export cancer type for R script
export CANCER_TYPE

# Run R script directly
R --vanilla --max-ppsize=500000 -f "./High_SRRM3_PSI_Analysis_by_Grade.R"
R_EXIT_CODE=$?
